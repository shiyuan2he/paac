...
import com.hsy.paac.urms.client.shiro.session.UpmsSessionDao;
 ...
 */
public class UpmsAuthenticationFilter {
    private final static Logger _logger = Logger.getLogger(UpmsAuthenticationFilter.class);
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        ...
                        _log.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(PAAC_URMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
